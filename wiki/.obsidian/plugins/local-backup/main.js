...
// src/constants.ts
var BACKUP_OUTPUT_PATH_ENV_KEY = "OBSIDIAN_LOCAL_BACKUP_PATH";
...
var LocalBackupSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Backup output path (optional)").setDesc(
      ...
      (text) => text.setValue(this.plugin.settings.backupOutputPathValue).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Included directories").setDesc(
      ...
      (text) => text.setPlaceholder(".obsidian").setValue(this.plugin.settings.includedDirectoriesValue).onChange(async (value) => {
...
var LocalBackupUtils = class {
   ...
   * @returns
   */
  deleteBackupsByLifeCycle(savePath, fileNameFormat, lifecycle) {
   ...
   * @param backupsPerDay
   */
  deletePerDayBackups(savePath, fileNameFormat, backupsPerDay) {
  ...
  async createZipByAdmZip(vaultPath, backupZipPath) {
    ...
    if ((includedPatterns.length > 0 || excludedPatterns.length > 0) && this.plugin.settings.showConsoleLog) {
    ...
    switch (archiverType) {
      case "sevenZip":
        const sevenZipPromise = new Promise((resolve, reject) => {
          ...
          (0, import_child_process.exec)(command, { cwd: includedPatterns.length > 0 ? vaultPath : void 0 }, (error, stdout, stderr) => {
            if (error) {
  ...
  parsePatternList(value) {
    return value.split(",").map((pattern) => pattern.trim()).filter((pattern) => pattern.length > 0);
  }
  collectIncludedEntries(vaultPath, includedPatterns, excludedPatterns) {
    if (includedPatterns.length === 0) {
      return [];
    }
    ...
    const walk = (dirPath, relativePath = "") => {
      ...
      for (const entry of entries) {
        ...
        if (stats.isDirectory() && this.shouldIncludePath(entryRelativePath, includedPatterns)) {
  ...
  matchesPatterns(filePath, patterns, actionLabel) {
    ...
    for (const pattern of patterns) {
      ...
      if (regex.test(normalizedPath)) {
        this.log(
          `${actionLabel} path: ${filePath} (matched pattern: ${pattern})`,
          "log"
        );
        return true;
      }
...
function getDefaultPath(app) {
  return path.dirname(app.vault.adapter.basePath);
}
function getDefaultName(app) {
  const vaultName = app.vault.getName();
  const defaultDatePlaceholders = getDatePlaceholdersForISO(true);
  return `${vaultName}-Backup-${defaultDatePlaceholders}`;
}
function readWindowsRegistryEnv(key) {
  const queries = [
  ...
  for (const cmd of queries) {
    try {
      const out = (0, import_child_process.execSync)(cmd, {
        encoding: "utf8",
        windowsHide: true,
        maxBuffer: 1024 * 1024
      });
...
function getBackupPathFromEnv() {
  var _a, _b, _c;
  const key = BACKUP_OUTPUT_PATH_ENV_KEY;
  let v = (_a = process.env[key]) == null ? void 0 : _a.trim();
  if (v) return v;
  if (process.platform === "win32") {
    ...
    if (found) {
      v = (_b = process.env[found]) == null ? void 0 : _b.trim();
      if (v) return v;
    }
    v = (_c = readWindowsRegistryEnv(key)) == null ? void 0 : _c.trim();
    if (v) return v;
  }
  return void 0;
}
function resolveBackupOutputPath(app, configuredPath) {
  const fromEnv = getBackupPathFromEnv();
  if (fromEnv) {
    return fromEnv;
  }
  const fromConfig = configuredPath == null ? void 0 : configuredPath.trim();
  if (fromConfig) {
    return fromConfig;
  }
...
var LocalBackupPlugin = class extends import_obsidian3.Plugin {
  ...
  async loadSettings() {
    ...
    if (raw && !("backupOutputPathValue" in raw) && ("winSavePathValue" in raw && raw.winSavePathValue || "unixSavePathValue" in raw && raw.unixSavePathValue)) {
      const legacy = process.platform === "win32" ? raw.winSavePathValue : raw.unixSavePathValue;
